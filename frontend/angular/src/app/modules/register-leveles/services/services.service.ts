...
export class ServicesService {
  ...
  getClientSecret(userId: string): Observable<any> {
    const headers = new HttpHeaders().set('Content-Type', 'application/json');
    const language = localStorage.getItem('languaje') || 'en'; // Usar inglés como predeterminado si no se encuentra el idioma

    return this.http.post(
      `${this.ulrStripe}subscriptions/create-setup-intent/${userId}?_locale=${language}`,
      {}, // Enviar un cuerpo vacío ya que no se necesita enviar datos adicionales
      { headers } // Pasar los encabezados aquí
    ).pipe(
      tap((response) => {
        console.log('Respuesta del backend (client secret):', response);
      })
    );
  }
